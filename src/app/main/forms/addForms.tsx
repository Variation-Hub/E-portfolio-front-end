...
import { createFormDataAPI, createUserFormDataAPI, getFormDataAPI, getUserFormDataAPI, selectFormData, slice, updateFormDataAPI } from "app/store/formData";
...
const AddForms = (props) => {
    const { data, formDataDetails, dataUpdatingLoadding, singleData, mode } = useSelector(selectFormData);
    ...
    const isFormData = Object.values(formData).find(data => data === "") === undefined;

    return (
        <Grid className="p-10 m-10">
            {user.role === UserRole.Admin && <Grid className="flex pb-10 mb-10 gap-10">
                <Grid className='w-full'>
                    <Typography sx={{ fontSize: "0.9vw", marginBottom: "0.5rem", fontWeight: "500" }}>Form Name</Typography>
                    <TextField
                        name="form_name"
                ...
                <Grid className='w-full'>
                    <Typography sx={{ fontSize: "0.9vw", marginBottom: "0.5rem", fontWeight: "500" }}>Select Type</Typography>
                    <Select
                        name="type"
                        value={formData?.type}
                        size="small"
            ...
            <Grid>
                {mode === 'view' || user.role !== UserRole.Admin ? (
                    ...
                    <FormBuilderIo
                        ...
                        onChange={schema => setFormData({ ...formData, form_data: schema.components })}
                    />
                )}
            </Grid>
            <Grid className="fixed bottom-0 right-0 p-14 w-full flex justify-end gap-10">
                ...
                {(dataUpdatingLoadding && user.role === UserRole.Admin) ?
                    ...
                    <>
                        ...
                        {(mode !== "view" && user.role === UserRole.Admin) &&
                            <SecondaryButton name={mode === "edit" ? "Update Form" : "Create Form"} onClick={handleSubmit} disable={!isFormData} />
                        }
                    </>
                }
            </Grid>
