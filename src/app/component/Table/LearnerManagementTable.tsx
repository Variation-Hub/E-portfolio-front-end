...
import { Autocomplete, Avatar, Box, Dialog, IconButton, Menu, MenuItem, Pagination, TextField, Typography } from '@mui/material';
...
import { DangerButton, LoadingButton, SecondaryButton, SecondaryButtonOutlined } from '../Buttons';
...
export default function LearnerManagementTable(props) {
    ...
    const deleteConfromation = async () => {
        await dispatch(deleteLearnerHandler(deleteId, meta_data, search_keyword, search_role));
        setDeleteId("");
    }
    ...
    const courseAllocation = async () => {
        setLoading(true)
        const response = await dispatch(courseAllocationAPI({ course_id: couseId, learner_id: openMenuDialog }))
        if (response) {
            clsoeCourseDialog()
            setOpenMenuDialog("")
            setCourseId("");
    ...
    return (
        <>
                                            ...
                                            if (column.id === "actions") {
                                                return (
                                                    <TableCell key={column.id} align={column.align}>
                                                        <IconButton size="small" sx={{ color: "#5B718F", marginRight: "4px" }} onClick={() => editIcon(row.learner_id)}>
                                                            <ModeEditOutlineOutlinedIcon fontSize='small' />
                                                        </IconButton>
                                                        <IconButton size="small" sx={{ color: "#5B718F", marginRight: "4px" }} onClick={(e) => openMenu(e, row.learner_id)}>
                                                            <MoreHorizIcon fontSize='small' />
                                                        </IconButton>
                                                        <IconButton size="small" sx={{ color: "maroon", marginLeft: "4px" }} onClick={() => deleteIcon(row.learner_id)}>
            ...
            <AlertDialog
                ...
                actionButton={dataUpdatingLoadding ? <LoadingButton /> : <DangerButton onClick={deleteConfromation} name="Delete learner" />}
            ...
            >
                ...
                <Box className="m-12 flex flex-col justify-between gap-12 sm:flex-row">
                    <div className='w-full'>
                        <Typography sx={{ fontSize: "0.9vw", marginBottom: "0.5rem" }}>Select Course</Typography>
                ...
                <div className="flex justify-end mt-4">
                    {loading ? <LoadingButton style={{ width: "10rem" }} />
                        :
                        <>
                            ...
                            />
                            <SecondaryButton name="Allocate" style={{ width: "10rem" }}
