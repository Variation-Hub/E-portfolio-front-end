...
const ResourceUploadDialog = (props) => {
  ...
  const handleChange = (file) => {
    setFile(file);
    setResourceData((prev) => ({ ...prev, name: file.name, size: (file.size / 1024 / 1024).toFixed(2) }));
  };
        ...
        <Box className="m-12 flex flex-col justify-between gap-12 sm:flex-row">
          ...
          <div className="w-1/3">
            ...
            <Autocomplete
              ...
              onChange={(e, value) => setResourceData(prev => ({ ...prev, job_type: value }))}
              sx={{
                '.MuiAutocomplete-clearIndicator': {
                  color: "#5B718F"
                }
        ...
        <Box className="m-12 flex flex-col justify-between gap-12 sm:flex-row">
          <div className="w-full">
            ...
            <Autocomplete
              fullWidth
              size="small"
              options={["PDF", "WORD", "PPT", "Text", "Image"].map((option) => option)}
              renderInput={(params) => <TextField {...params} placeholder="Select type" name="role" />}
              onChange={(e, value) => setResourceData(prev => ({ ...prev, resource_type: value }))}
              sx={{
                '.MuiAutocomplete-clearIndicator': {
                  color: "#5B718F"
                }
          ...
          {loading ? <LoadingButton style={{ width: "10rem" }} />
            ...
            <>
              <SecondaryButtonOutlined
                ...
                onClick={handleClose}
              />
              <SecondaryButton name="Create" style={{ width: "10rem" }} onClick={recouseHandler} />
            </>
          }
        </div>
      </div>
